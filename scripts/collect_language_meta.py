...
table_directories = ["../data/langtables", "../data/langtables_structural"]
missing_tables = [
    language
    for language in d["language"]
    if not any(
        os.path.isfile(os.path.join(directory, f"{language}_data.csv"))
        for directory in table_directories
    )
]
if missing_tables:
    raise FileNotFoundError(
        f'No data table found for languages: {", ".join(missing_tables)}'
    )
